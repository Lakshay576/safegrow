...
export default function Sidebar({ role, onNavigate }: Props) {
  ...
  return (
    <nav className="w-full bg-[var(--surface)] px-3 py-4 space-y-1">
      {menu[role].map((item) => {
        ...
        return (
          <Link
            ...
            className={`
              ...
              ${isActive
                ? "bg-[var(--primary-lt)] font-semibold text-[var(--primary)]"
                : "text-[var(--ink-muted)] font-medium hover:bg-[var(--secondary-lt)] hover:text-[var(--ink)]"
              }
            `}
          >
            <span className={isActive ? "text-[var(--primary)]" : "text-[var(--ink-muted)]"}>
              {item.icon}
            </span>
            {item.name}
          </Link>
